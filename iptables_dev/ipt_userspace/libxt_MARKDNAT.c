...
static const struct xt_option_entry markdnat_tg_opts[] = {
...
static void markdnat_tg_help(void)
{
	printf(
...
static void markdnat_tg_parse(struct xt_option_call *cb)
{
...
static void markdnat_tg_check(struct xt_fcheck_call *cb)
{
	if (cb->xflags == 0)
...
static void markdnat_tg_print(const void *ip, const struct xt_entry_target *target,
                          int numeric)
{
...
static void markdnat_tg_save(const void *ip, const struct xt_entry_target *target)
{
	const struct xt_mark_tginfo2 *info = (const void *)target->data;
...
static struct xtables_target markdnat_tg_reg = {
	...
	.help          = markdnat_tg_help,
	.print         = markdnat_tg_print,
	.save          = markdnat_tg_save,
	.x6_parse      = markdnat_tg_parse,
	.x6_fcheck     = markdnat_tg_check,
	.x6_options    = markdnat_tg_opts,
};

void _init(void)
{
	xtables_register_target(&markdnat_tg_reg);
}
