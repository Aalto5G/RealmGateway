...
LOGLEVELADDRESSPOOL = logging.WARNING


def _get_address_pool(addrmask, ipv6=False):
    """ 
...
class AddressPoolContainer(container3.Container):
    
    def __init__(self, name='AddressPoolContainer'):
        """ Initialize the AddressPoolContainer as a Container """
        super().__init__(name, LOGLEVELADDRESSPOOL)

class AddressPoolShared(container3.ContainerNode):
    def __init__(self, key, addrmask=None, name='AddressPoolShared'):
        """ Initialize the AddressPoolShared as a ContainerNode """
        super().__init__(name, LOGLEVELADDRESSPOOL)
        self._key = key
...
class AddressPoolUser(container3.ContainerNode):
    def __init__(self, key, addrmask=None, name='AddressPoolUser'):
        """ Initialize the AddressPoolUser as a ContainerNode """
        super().__init__(name, LOGLEVELADDRESSPOOL)
        self._key = key
        self._pool = {}
        self._addrpool = []
...
class _AddressPoolUnit(object):
    def __init__(self, name='_AddressPoolUnit'):
        ...
        self._logger.setLevel(LOGLEVELADDRESSPOOL)
        self._pool = []
        self._allocated = []
        self._available = []
    ...
    def add_to_pool(self, addrmask):
        ...
        for addr in _get_address_pool(addrmask):
