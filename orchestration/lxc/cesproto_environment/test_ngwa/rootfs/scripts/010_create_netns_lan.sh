...
LAST="250"
NIC="lan0"
IPADDR_PREFIX="192.168.10."
IPADDR_NETPREFIX="/24"
IPADDR_GATEWAY="192.168.10.1"
IPADDR_DNS="192.168.10.1"
...
for i in `seq $FIRST $LAST`; do
    ...
    ip netns exec $nsname sysctl -w "net.ipv4.conf.all.accept_redirects=1"    > /dev/null 2> /dev/null
    ip netns exec $nsname sysctl -w "net.ipv4.ip_local_port_range=1024 65535" > /dev/null 2> /dev/null
    ip netns exec $nsname sysctl -w "net.ipv4.tcp_fin_timeout=2"              > /dev/null 2> /dev/null
    ...
    #Send Gratuitous ARP
    ip netns exec $nsname arping -c 2 -w 0.5 -U -I $NIC $IPADDR_PREFIX$i
done
