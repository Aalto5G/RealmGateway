...
### LAN & WAN
iptables -t mangle -A FORWARD -i $LAN_NIC   -s $LAN_NET ! -d $PROXY_NET -j MARK --set-mark $hMARK_EGRESS_to_WAN
iptables -t mangle -A FORWARD -i $WAN_NIC ! -s $PROXY_NET -d $LAN_NET   -j MARK --set-mark $hMARK_INGRESS_from_WAN
### LAN & CES
iptables -t mangle -A FORWARD -i $LAN_NIC   -s $LAN_NET   -d $PROXY_NET -j MARK --set-mark $hMARK_EGRESS_to_PROXY
iptables -t mangle -A FORWARD -i $VTEP_NIC  -s $PROXY_NET -d $LAN_NET   -j MARK --set-mark $hMARK_INGRESS_from_PROXY
### LAN & WAN via CES input
iptables -t mangle -A INPUT   -i $LAN_NIC                               -j MARK --set-mark $hMARK_EGRESS_to_CES
iptables -t mangle -A INPUT   -i $WAN_NIC                               -j MARK --set-mark $hMARK_INGRESS_from_CES
...
iptables -A CES_POLICY_DNS -m mark --mark $hMARK_INGRESS_from_CES -p udp --dport 53 -m limit --limit 2/s --limit-burst 3 -j RETURN
iptables -A CES_POLICY_DNS -m mark --mark $hMARK_INGRESS_from_CES -p udp --dport 53 -m limit --limit 1/s --limit-burst 1 -j LOG --log-prefix "Exceeded DNS WAN: " --log-level 7
iptables -A CES_POLICY_DNS -m mark --mark $hMARK_INGRESS_from_CES -p udp --dport 53 -j doREJECT
iptables -A CES_POLICY_DNS -m mark --mark $hMARK_INGRESS_from_CES -p tcp --dport 53 -m limit --limit 2/s --limit-burst 3 -j RETURN
iptables -A CES_POLICY_DNS -m mark --mark $hMARK_INGRESS_from_CES -p tcp --dport 53 -m limit --limit 1/s --limit-burst 1 -j LOG --log-prefix "Exceeded DNS WAN: " --log-level 7
iptables -A CES_POLICY_DNS -m mark --mark $hMARK_INGRESS_from_CES -p tcp --dport 53 -j doREJECT
...
iptables -A CES_POLICY_HTTP -m mark --mark $hMARK_INGRESS_from_CES -p tcp -m state --state RELATED,ESTABLISHED -j RETURN
iptables -A CES_POLICY_HTTP -m mark --mark $hMARK_INGRESS_from_CES -p tcp --dport 80  -m state --state NEW     -j RETURN
iptables -A CES_POLICY_HTTP -m mark --mark $hMARK_INGRESS_from_CES -p tcp --dport 443 -m state --state NEW     -j RETURN
...
iptables -A HOST_192.168.0.101 -m mark --mark $hMARK_EGRESS_to_PROXY -d 172.16.0.1 -j HOST_192.168.0.101_SERVICE1
iptables -A HOST_192.168.0.101 -m mark --mark $hMARK_INGRESS_from_PROXY  -s 172.16.0.1 -j HOST_192.168.0.101_SERVICE1
iptables -A HOST_192.168.0.101 -m mark --mark $hMARK_EGRESS_to_PROXY -d 172.16.0.2 -j HOST_192.168.0.101_SERVICE2
iptables -A HOST_192.168.0.101 -m mark --mark $hMARK_INGRESS_from_PROXY  -s 172.16.0.2 -j HOST_192.168.0.101_SERVICE2

## Reject unallocated outgoing traffic to PROXY_NET
iptables -A HOST_192.168.0.101 -m mark --mark $hMARK_EGRESS_to_PROXY -j doREJECT
